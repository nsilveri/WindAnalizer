...
class SDCard:
    ...
    def mkfs(self):
        """Format the SD card with a FAT filesystem using uos.VfsFat.mkfs.

        This requires the port to provide `uos.VfsFat.mkfs`. The SDCard
        instance implements the block device API expected by mkfs.
        """
        try:
            import uos
        except Exception:
            raise OSError("uos module not available")

        try:
            vfs = uos.VfsFat
        except AttributeError:
            raise OSError("VfsFat.mkfs not available on this port")
    ...
    # convenience alias
    def format_sd(self):
        """Alias for `mkfs` to make intent explicit in user code."""
        return self.mkfs()