...
PinOn = Pin(14, Pin.OUT)  # for easier reference below
#PinOn.on()  # set high
...
def create_tof(i2c=None, lpn_pin=None, lpn_obj=None):
    ...
    if lpn_obj is None:
        if _CIRCUITPY:
            ...
            if lpn_pin is None:
                lpn = PinOn #_NoopLpn(initial=True)
            else:
                try:
                    from machine import Pin

                    if isinstance(lpn_pin, Pin):
                        print("Using provided machine.Pin for LPN")
                        p = lpn_pin
                        # Ensure provided Pin is configured as an output so
                        # the driver can toggle it for reset.
                        try:
                            p.init(Pin.OUT)
                        except Exception:
                            try:
                                # Some ports accept keyword args
                                p.init(mode=Pin.OUT)
                            except Exception:
                                pass
                    else:
                        print("Creating machine.Pin for LPN on pin", lpn_pin)
...
def run_test_loop(i2c=None, lpn_pin=None, interval_s=0.5, iterations=None):
    ...
    """
    # Create TOF device — ensure test LPN pin is an output
    tof = create_tof(i2c=i2c_m, lpn_pin=Pin(14, Pin.OUT))
